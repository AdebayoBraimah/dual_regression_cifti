...
out_dir_agg=${agg_dir}/seed-to-voxel.analysis.19_Aug_2020/${design_name}
out_dir_nonagg=${nonagg_dir}/seed-to-voxel.analysis.19_Aug_2020/${design_name}
...
ic_map=/scratch/brac4g/CAP/BIDS/scripts/cifti_recon/cifti.ica/ROIs/ROIs.8/ROIs.8.dscalar.nii
jobs=10
perm=5000
...
echo ""
bsub -N -o ${o_agg} -e ${e_agg} -q ${l_queue} -M ${l_mem} -W ${l_wall} -J DR_agg ${scripts_dir}/dual_regression_cifti.sh --queue ${queue} --surf-list-L ${L_s_list} --surf-list-R ${R_s_list} --ica-maps ${ic_map} --file-list ${sub_list_agg} --out-dir ${out_dir_agg} --atlas-dir ${atlas_dir} --jobs ${jobs} --des-norm --design ${design} --t-contrast ${contrast} --permutations ${perm} --fdr --log-p --memory ${mem} --wall ${wall} --convert-all --sig 0.05 --method sid --resub --precision double --no-stats-cleanup
